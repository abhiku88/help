...
class Tickets extends Database {  
	...
	public function showTickets(){
		...
		$sqlQuery = "SELECT t.id, t.uniqid, t.title, t.init_msg as message, t.date, t.last_reply, t.resolved, u.name as creater, d.name as department, u.user_type, t.user, t.user_read, t.admin_read
			FROM hd_tickets AS t
			LEFT JOIN hd_users AS u ON t.user = u.id 
			LEFT JOIN hd_departments AS d ON t.department = d.id $sqlWhere ";
		if(!empty($_POST["search"]["value"])){
	...
	public function createTicket() {      
		if(!empty($_POST['subject']) && !empty($_POST['message'])) {                
			...
			VALUES('".$uniqid."', '".$_SESSION["userid"]."', '".$_POST['subject']."', '".$message."', '".$_POST['department']."', '"."', '".$_SESSION["userid"]."', 0, 0, '".$_POST['status']."')";			
